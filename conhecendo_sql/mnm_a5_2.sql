...
#ID do Aluno: Um identificador único para cada aluno. Nome do Aluno: O nome completo do aluno. Data de Nascimento: A data de nascimento do aluno. Gênero: O gênero do aluno (masculino, feminino, outros). Endereço: O endereço do aluno. Telefone de Contato: O número de telefone de contato do aluno. E-mail: O endereço de e-mail do aluno. Tabela "Professores" conterá detalhes sobre os professores da escola:

#ID do Professor: Um identificador único para cada professor. Nome do Professor: O nome completo do professor. Data de Nascimento: A data de nascimento do professor. Gênero: O gênero do professor. Telefone de Contato: O número de telefone de contato do professor. E-mail: O endereço de e-mail do professor. Tabela "Disciplinas" manterá registros das matérias oferecidas, incluindo:

#ID da Disciplina: Um identificador único para cada disciplina. Nome da Disciplina: O nome da disciplina. Descrição: Uma descrição da disciplina. Carga Horária: A carga horária da disciplina. ID do Professor: Uma chave estrangeira que faz referência ao professor que leciona a disciplina. Tabela "Turmas" será usada para registrar turmas específicas:

#ID da Turma: Um identificador único para cada turma. Nome da Turma: O nome ou código da turma. Ano Letivo: O ano letivo da turma. ID do Professor Orientador: Uma chave estrangeira que faz referência ao professor que orienta a turma. Tabela "Turma_Disciplinas" armazenara os dados das associações entre turmas e disciplinas:
...
#ID da Turma: Uma lista de chaves estrangeiras que fazem referência as turmas existentes. ID dos Alunos: Uma lista de chaves estrangeiras que fazem referência aos alunos matriculados na turma. OBS.: Os dois campos juntos formam a chave primaria da tabela

#Tabela "Notas" guardará as notas dos alunos em diferentes disciplinas:

#ID da Nota: Um identificador único para cada nota. ID do Aluno: Uma chave estrangeira que faz referência ao aluno. ID da Disciplina: Uma chave estrangeira que faz referência à disciplina. Valor da Nota: A nota atribuída ao aluno na disciplina. Data da Avaliação: A data em que a avaliação foi realizada. 
...
CREATE TABLE alunos (
  ...
  tel_contato_aluno	VARCHAR(20),
  email_aluno		VARCHAR(100)
  
  CONSTRAINT chk_genero CHECK (genero_aluno IN ('Masculino', 'Feminino'))
  );

CREATE TABLE professores (
  id_professor			INT PRIMARY KEY,
  nome_aprofessor		VARCHAR(100),
  data_nasc_professor	DATE,
  genero_professor		CHAR(1),
  endereco_professor	VARCHAR(150),
  tel_contato_professor	VARCHAR(20),
...
#Depois de criar as tabelas, é hora de inserir os dados. Para isso, você pode baixar aqui neste link (arquivo-desafio), todos os arquivos com os dados para popular todas as tabelas.

INSERT INTO 'alunos' ('id_aluno','nome_aluno','data_nasc_aluno','genero_aluno','endereco_aluno','tel_contato_aluno','email_aluno') VALUES 
...
INSERT INTO 'professores' ('id_professor','nome_professor','data_nasc_professor','genero_professor','tel_contato_professor','email_professor') VALUES 
...
INSERT INTO 'disciplina' ('id_disciplina','nome_disciplina','descricao_disciplina','carga_horaria','id_professor') VALUES 
